...
# selected x1 SCSAHDS outcome, as germaine demand in the revision
...
mvlm_ml <- function(indvar, dat) {
    setform <- sprintf("log(SCSAHDS + 1e-10) ~ I(scale(log10(%s+1))) + lipids_m + Age_m + catbmi_m + LMSMKNOW + LMEXERCS + parity_m", indvar)
...
mvlm_mc <- function(indvar, dat) {
    setform <- sprintf("log(SCSAHDS + 1e-10) ~ I(scale(log10(%s+1))) + CREAMOUNT_m + Age_m + catbmi_m + LMSMKNOW + LMEXERCS + parity_m", indvar)
    mod <- with(data = dat, lm(as.formula(setform)))
}

mvlm_mn <- function(indvar, dat) {
    setform <- sprintf("log(SCSAHDS + 1e-10) ~ I(scale(log10(%s+1))) + Age_m + catbmi_m + LMSMKNOW + LMEXERCS + parity_m", indvar)
    mod <- with(data = dat, lm(as.formula(setform)))
}
...
# save(ret_list_full, file = "results/sim_stain_SCSAHDS.RData", compress = FALSE)
# save(ret_df_full_r2_adj, file = "results/sim_r2_stain_SCSAHDS.RData", compress = FALSE)
...
## Male
mvlm_ml <- function(indvar, dat) {
    setform <- sprintf("log(SCSAHDS + 1e-10) ~ lipids_m + Age_m + catbmi_m + LMSMKNOW + LMEXERCS + parity_m", indvar)
    mod <- with(data = dat, lm(as.formula(setform)))
}


mvlm_mc <- function(indvar, dat) {
    setform <- sprintf("log(SCSAHDS + 1e-10) ~ CREAMOUNT_m + Age_m + catbmi_m + LMSMKNOW + LMEXERCS + parity_m", indvar)
    mod <- with(data = dat, lm(as.formula(setform)))
}

mvlm_mn <- function(indvar, dat) {
    setform <- sprintf("log(SCSAHDS + 1e-10) ~ Age_m + catbmi_m + LMSMKNOW + LMEXERCS + parity_m", indvar)
    mod <- with(data = dat, lm(as.formula(setform)))
}
...
outfilename <- "mv_EWAS_power_semen_chems_SCSAHDS.Rdata"
...
# load("results/mv_EWAS_power_semen_chems_SCSAHDS.Rdata") 
...
tmp1$EDC <- c("oxychlordane", "PBDE 17", "2,2′-OH-4-MeO-BP", "Me-PFOSA-AcOH", "PFOSA") # manual

# plot
p <- ggplot(powerplot_df, aes(sample_size, power, group = EDC))
p <- p + geom_line(aes(colour = factor(EDC_class))) + xlim(300, 750)
p <- p + colorCodes_gg
p <- p + ggtitle("Power vs sample size for the endpoint DNA stainability") +
    xlab("Sample size") + 
...
p <- p + geom_text_repel(data = tmp1, aes(label = EDC), 
                         ...
                         nudge_x = 30, 
                         nudge_y = ifelse(tmp1$EDC %in% c("2,2′-OH-4-MeO-BP", "PBDE 17"), -0.01, 0.025))
p

ggsave("results/power_n_SCSAHDS.png", scale=1, dpi=400)
# ggsave("results/power_n_SCSAHDS.svg", scale=1, dpi=400)
