# July 17 2017
## Goal: EWAS with lm of continous male Y
...
mvlm_ml <- function(indvar, dat) {
    setform <- sprintf("cbind(WHONORM, log(SEVOLUME + 1e-10), log(SPCOUNT + 1e-10), log(TOTCNT + 1e-10), log(PERMOT + 1e-10), log(SCSADFI + 1e-10), log(SCSAHDS + 1e-10)) ~ I(scale(log10(%s+1))) + lipids_m + Age_m + catbmi_m + LMSMKNOW + LMEXERCS + parity_m", indvar)
    mod <- with(data = dat, lm(as.formula(setform)))
}


mvlm_mc <- function(indvar, dat) {
    setform <- sprintf("cbind(WHONORM, log(SEVOLUME + 1e-10), log(SPCOUNT + 1e-10), log(TOTCNT + 1e-10), log(PERMOT + 1e-10), log(SCSADFI + 1e-10), log(SCSAHDS + 1e-10)) ~ I(scale(log10(%s+1))) + CREAMOUNT_m + Age_m + catbmi_m + LMSMKNOW + LMEXERCS + parity_m", indvar)
    mod <- with(data = dat, lm(as.formula(setform)))
}

mvlm_mn <- function(indvar, dat) {
    setform <- sprintf("cbind(WHONORM, log(SEVOLUME + 1e-10), log(SPCOUNT + 1e-10), log(TOTCNT + 1e-10), log(PERMOT + 1e-10), log(SCSADFI + 1e-10), log(SCSAHDS + 1e-10)) ~ I(scale(log10(%s+1))) + Age_m + catbmi_m + LMSMKNOW + LMEXERCS + parity_m", indvar)
    mod <- with(data = dat, lm(as.formula(setform)))
}
...
# function to extract what I need from mlm.ml class
outmanova <- car:::print.Anova.mlm
body(outmanova)[[16]] <- quote(invisible(tests))
body(outmanova)[[15]] <- NULL

# extraction function
mvlm_jake <- function (object) {    
    analyses <- object$analyses
    m <- length(analyses)
    coef1 <- matrix(NA, nrow = m, ncol = 3, dimnames = list(1:m, c("F", "df1_num", "df2_den")))
    for (i in 1:m) {
        fit <- analyses[[i]]
        coef1[i, 1:3] <- as.matrix(outmanova(Manova(fit, type = "II"))[1, 3:5]) # Type II MANOVA Tests: Pillai test statistic
    }
    return(coef1)
}
...
ret_df_mvlm <- data.frame()

for(i in 1:length(unlist(cl_m))) {
    tmpm_var <- unlist(cl_m)[i]
    input_m <- ret_list[[tmpm_var]]
    mat <- mvlm_jake(input_m)
    frm <- as.data.frame(t(micombine.F(mat[,1], mat[1,2])))
    frm$indvar <- tmpm_var
    print(tmpm_var)
    ret_df_mvlm <- rbind(ret_df_mvlm, frm)
}
...
## tmp FDR check
tmpa <- ret_df_mvlm
tmpa$fdr <- p.adjust(tmpa$p, method='fdr')  # min(fdr)
tmpa %>% arrange(fdr)
tmpa %>%  filter(fdr <= 0.05) %>% arrange(fdr)
