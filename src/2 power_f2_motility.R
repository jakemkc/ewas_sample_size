...
mvlm_ml <- function(indvar, dat) {
    setform <- sprintf("log(PERMOT + 1e-10) ~ I(scale(log10(%s+1))) + lipids_m + Age_m + catbmi_m + LMSMKNOW + LMEXERCS + parity_m", indvar)
...
mvlm_mc <- function(indvar, dat) {
    setform <- sprintf("log(PERMOT + 1e-10) ~ I(scale(log10(%s+1))) + CREAMOUNT_m + Age_m + catbmi_m + LMSMKNOW + LMEXERCS + parity_m", indvar)
    mod <- with(data = dat, lm(as.formula(setform)))
}

mvlm_mn <- function(indvar, dat) {
    setform <- sprintf("log(PERMOT + 1e-10) ~ I(scale(log10(%s+1))) + Age_m + catbmi_m + LMSMKNOW + LMEXERCS + parity_m", indvar)
    mod <- with(data = dat, lm(as.formula(setform)))
}
...
# save(ret_list_full, file = "results/sim_motility_PERMOT.RData", compress = FALSE)
# save(ret_df_full_r2_adj, file = "results/sim_r2_motility_PERMOT.RData", compress = FALSE)
...
## Male
mvlm_ml <- function(indvar, dat) {
    setform <- sprintf("log(PERMOT + 1e-10) ~ lipids_m + Age_m + catbmi_m + LMSMKNOW + LMEXERCS + parity_m", indvar)
    mod <- with(data = dat, lm(as.formula(setform)))
}


mvlm_mc <- function(indvar, dat) {
    setform <- sprintf("log(PERMOT + 1e-10) ~ CREAMOUNT_m + Age_m + catbmi_m + LMSMKNOW + LMEXERCS + parity_m", indvar)
    mod <- with(data = dat, lm(as.formula(setform)))
}

mvlm_mn <- function(indvar, dat) {
    setform <- sprintf("log(PERMOT + 1e-10) ~ Age_m + catbmi_m + LMSMKNOW + LMEXERCS + parity_m", indvar)
    mod <- with(data = dat, lm(as.formula(setform)))
}
...
outfilename <- "mv_EWAS_power_semen_chems.Rdata"
...
# load("results/mv_EWAS_power_semen_chems.Rdata") 
...
# quantile
quantile(es$power2)
quantile(es$power2, 0.95)

# write
# write.csv(es, file = "results/power_bon_adj_p.csv") 


## \\ cohen-n ----

pwr.f2.test(u = 1, f2 = 0.02, sig.level = 0.05/128, power = 0.8) 

pwr.f2.test(u = 1, f2 = 0.02, sig.level = 0.05/10, power = 0.8)

# summary(es$f2)
quantile(es$f2, 0.95)
pwr.f2.test(u = 1, f2 = 0.009187434, sig.level = 0.05/128, power = 0.8) 

quantile(es$f2, 0.5)
pwr.f2.test(u = 1, f2 = 9.890e-04, sig.level = 0.05/128, power = 0.8) 
...
# name list for germaine paper (x15 categories)
# 3.1. Create list for matching in the loop to choose model
...
tmp1$EDC <- c("PCB 99", "PBDE 66", "PCB 105", "PCB 114", "PCB 167") # manual

# plot
p <- ggplot(powerplot_df, aes(sample_size, power, group = EDC))
p <- p + geom_line(aes(colour = factor(EDC_class))) + xlim(300, 750)
p <- p + colorCodes_gg
p <- p + ggtitle("Power vs sample size for the endpoint motility") +
    xlab("Sample size") + 
...
ggsave("results/power_n_motility.png", scale=1, dpi=400)
# ggsave("results/power_n_motility.svg", scale=1, dpi=400)
...
# \\ cp-power-plot-2 ----
# To show power vs p value, using motility as endpoint, show relationship at n = 501

# power vs sample size
cp_p_list <- list()
cpseq <- 501 # selected at 501
...
# merge p value
pind <- sapply(es$indvar, function(x) match(x, powerplot_df$EDC)) # find the matching index
powerplot_df2 <- cbind(es, powerplot_df[pind,])
...
for(i in 1:length(cl_m)) {
    mtchC <- which(powerplot_df2$EDC %in% cl_m[[i]])
    EDC_class[mtchC] <- names(cl_m)[i]
}

powerplot_df2 <- cbind(powerplot_df2, EDC_class)

# create var for ordering x axis (thru factor lv)
powerplot_df2$EDC <- factor(powerplot_df2$EDC, levels = unlist(cl_m), ordered = TRUE) # manhat X in my order
powerplot_df2$EDC_class <- factor(powerplot_df2$EDC_class, levels = names(cl_m), ordered = TRUE) # manhat legend in my order
...
tmp1 <- tmp1[order(tmp1[,4]),]
tmp1 <- head(tmp1, 5)
tmp1$EDC <- c("CB_167", "CB_114", "CB_105", "CB_99", "BDE_66")


p <- ggplot(powerplot_df2, aes(pvaledc, power))
p <- p + geom_point(aes(colour = EDC_class))
p <- p + colorCodes_gg
p <- p + geom_smooth(span = 0.8, formula = y ~ log(x), se = FALSE, size = 0.5)
p <- p + geom_text_repel(data = tmp1, aes(label = EDC), nudge_x = 0.11, nudge_y = 0.025, size = 4)
p


ggsave("results/power_p_val_motility.png", scale=1, dpi=400)
