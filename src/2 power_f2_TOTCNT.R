...
# selected x1 TOTCNT outcome, as germaine demand in the revision
...
mvlm_ml <- function(indvar, dat) {
    setform <- sprintf("log(TOTCNT + 1e-10) ~ I(scale(log10(%s+1))) + lipids_m + Age_m + catbmi_m + LMSMKNOW + LMEXERCS + parity_m", indvar)
...
mvlm_mc <- function(indvar, dat) {
    setform <- sprintf("log(TOTCNT + 1e-10) ~ I(scale(log10(%s+1))) + CREAMOUNT_m + Age_m + catbmi_m + LMSMKNOW + LMEXERCS + parity_m", indvar)
    mod <- with(data = dat, lm(as.formula(setform)))
}

mvlm_mn <- function(indvar, dat) {
    setform <- sprintf("log(TOTCNT + 1e-10) ~ I(scale(log10(%s+1))) + Age_m + catbmi_m + LMSMKNOW + LMEXERCS + parity_m", indvar)
    mod <- with(data = dat, lm(as.formula(setform)))
}
...
# save(ret_list_full, file = "results/sim_total_count_TOTCNT.RData", compress = FALSE)
# save(ret_df_full_r2_adj, file = "results/sim_r2_total_count_TOTCNT.RData", compress = FALSE)
...
## Male
mvlm_ml <- function(indvar, dat) {
    setform <- sprintf("log(TOTCNT + 1e-10) ~ lipids_m + Age_m + catbmi_m + LMSMKNOW + LMEXERCS + parity_m", indvar)
    mod <- with(data = dat, lm(as.formula(setform)))
}
...
mvlm_mc <- function(indvar, dat) {
    setform <- sprintf("log(TOTCNT + 1e-10) ~ CREAMOUNT_m + Age_m + catbmi_m + LMSMKNOW + LMEXERCS + parity_m", indvar)
    mod <- with(data = dat, lm(as.formula(setform)))
}

mvlm_mn <- function(indvar, dat) {
    setform <- sprintf("log(TOTCNT + 1e-10) ~ Age_m + catbmi_m + LMSMKNOW + LMEXERCS + parity_m", indvar)
    mod <- with(data = dat, lm(as.formula(setform)))
}
...
outfilename <- "mv_EWAS_power_semen_chems_TOTCNT.Rdata"
...
# load("results/mv_EWAS_power_semen_chems_TOTCNT.Rdata") 
...
tmp1$EDC <- c("HP", "g-HCH", "O-DMA", "thallium", "Me-PFOSA-AcOH") # manual

# plot
p <- ggplot(powerplot_df, aes(sample_size, power, group = EDC))
p <- p + geom_line(aes(colour = factor(EDC_class))) + xlim(300, 750)
p <- p + colorCodes_gg
p <- p + ggtitle("Power vs sample size for the endpoint total sperm count") +
    xlab("Sample size") + 
...
p <- p + geom_text_repel(data = tmp1, aes(label = EDC),
                         ...
                         force = 40, # Strength of the repulsion force.
                         nudge_x = ifelse(tmp1$EDC %in% c("Me-PFOSA-AcOH"), -10, 35),
                         nudge_y = 0.005)
p

ggsave("results/power_n_TOTCNT.png", scale=1, dpi=400)
# ggsave("results/power_n_TOTCNT.svg", scale=1, dpi=400)
