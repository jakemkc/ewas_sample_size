...
mvlm_ml <- function(indvar, dat) {
    setform <- sprintf("WHONORM ~ I(scale(log10(%s+1))) + lipids_m + Age_m + catbmi_m + LMSMKNOW + LMEXERCS + parity_m", indvar)
    mod <- with(data = dat, lm(as.formula(setform)))
}


mvlm_mc <- function(indvar, dat) {
    setform <- sprintf("WHONORM ~ I(scale(log10(%s+1))) + CREAMOUNT_m + Age_m + catbmi_m + LMSMKNOW + LMEXERCS + parity_m", indvar)
    mod <- with(data = dat, lm(as.formula(setform)))
}

mvlm_mn <- function(indvar, dat) {
    setform <- sprintf("WHONORM ~ I(scale(log10(%s+1))) + Age_m + catbmi_m + LMSMKNOW + LMEXERCS + parity_m", indvar)
    mod <- with(data = dat, lm(as.formula(setform)))
}
...
# save(ret_list_full, file = "results/sim_morphology_WHONORM.RData", compress = FALSE)
# save(ret_df_full_r2_adj, file = "results/sim_r2_morphology_WHONORM.RData", compress = FALSE)
...
## Male
mvlm_ml <- function(indvar, dat) {
    setform <- sprintf("WHONORM ~ lipids_m + Age_m + catbmi_m + LMSMKNOW + LMEXERCS + parity_m", indvar)
    mod <- with(data = dat, lm(as.formula(setform)))
}
...
mvlm_mc <- function(indvar, dat) {
    setform <- sprintf("WHONORM ~ CREAMOUNT_m + Age_m + catbmi_m + LMSMKNOW + LMEXERCS + parity_m", indvar)
    mod <- with(data = dat, lm(as.formula(setform)))
}

mvlm_mn <- function(indvar, dat) {
    setform <- sprintf("WHONORM ~ Age_m + catbmi_m + LMSMKNOW + LMEXERCS + parity_m", indvar)
    mod <- with(data = dat, lm(as.formula(setform)))
}
...
# remove -ve f2
es <- es[(es$f2 > 0),] # remove 2: nickel_m, mEHP_m; 
# error from regression: Warning: contrasts dropped from factor parity_m due to missing levels

# dir
outdirectory <- "results"
outfilename <- "mv_EWAS_power_semen_chems_morphology.Rdata"
...
# load("results/mv_EWAS_power_semen_chems_morphology.Rdata") 
...
pwr.f2.test(u = 1, f2 = 0.008830002, sig.level = 0.05/128, power = 0.8)
...
# summary(es$f2)
pwr.f2.test(u = 1, f2 = 1.263e-03, sig.level = 0.05/128, power = 0.8) # on date 111517 to answer germaine's revision
...
for(i in 1:nrow(es)) { # avoid nested loop; apply can take list() and output a list()
    input_1 <- es$f2[i]
...
# setdiff(colnames(lifemerg), names(papername_m))
...
tmp1 <- tail(tmp1, 5)
tmp1$EDC <- c("PBDE 17", "mCMHP", "mEHP", "OH-Et-P", "mNP") # manual

# plot
p <- ggplot(powerplot_df, aes(sample_size, power, group = EDC))
p <- p + geom_line(aes(colour = factor(EDC_class))) + xlim(300, 750)
p <- p + colorCodes_gg
p <- p + ggtitle("Power vs sample size for the endpoint morphology") +
    xlab("Sample size") + 
...
ggsave("results/power_n_morphology.png", scale=1, dpi=400)
# ggsave("results/power_n_morphology.svg", scale=1, dpi=400)
